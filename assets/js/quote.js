$(document).ready(function () {
    ...
    function setPricesResults(finalNumElev, subTotal, installationFee, grandTotal) {
        $("#elevTotal").val(parseFloat(subTotal).toFixed(2) + " $");
        $("#installationFee").val(parseFloat(installationFee).toFixed(2) + " $");
        $("#total_").val(parseFloat(grandTotal).toFixed(2) + " $");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (prodRange.type != null) {
                    ...
                    console.log('finalnumelevators'+data.finalNumElev)
                    console.log('installfee'+data.installationFee)
                    console.log('gtotal'+data.grandTotal)
    ...
    function doCalc() {
        if ($('#residential').hasClass('active') && !negativeValues() && $('#numApp').val() && $('#numFloors').val()) {
            ...
            apiCall('commercial')
        console.log('numberofElevators'+$('#numElev').val())

        console.log('numberofFloors'+$('#numFloors').val())

        console.log('numberofBasements'+$('#numBase').val())
