$(document).ready(function () {
  ...
  function setRequiredElevatorsResult(finalNumElev) {
    $("#numElev_2, #numElev_3").val(parseFloat(finalNumElev));
  }

  function setPricesResults(roughTotal, installFee, total_) {
    $("#elevTotal").val(parseFloat(roughTotal).toFixed(2) + " $");
    $("#installationFee").val(parseFloat(installFee).toFixed(2) + " $");
    $("#total_").val(parseFloat(total_).toFixed(2) + " $");
  }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (prodRange.type != null) {
          // si prodRange n'est pas de type nulle, alors fais la fonction setPriceResults.
          setPricesResults(
            //pour chacun de ses paramètres, il faut leur donner des arguments.
            data.subTotal,
