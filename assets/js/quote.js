$(document).ready(function () {
  ...
  $("#standard, #premium, #excelium").on("click", function () {
    if ($("#standard").is(":checked")) {
      document.getElementById("elevPriceUnit").value = (7565).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $";
      doCalc();
    } else if ($("#premium").is(":checked")) {
      document.getElementById("elevPriceUnit").value = (12345).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $";
      doCalc();
    } else if ($("#excelium").is(":checked")) {
      document.getElementById("elevPriceUnit").value = (15400).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $";
      doCalc();
    }
  });
  ...
  function setPricesResults(finNumElev, roughTotal, installFee, total) {
    $("#elevTotal").val(
      parseFloat(roughTotal)
        .toFixed(2)
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $"
    );
    $("#installationFee").val(
      parseFloat(installFee)
        .toFixed(2)
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $"
    );
    $("#total_").val(
  ...
  function doCalc() {
    if ($("#residential").hasClass("active") && !negativeValues() && $("#numApp").val() && $("#numFloors").val()) {
      console.log(numElev, "JE NE SUIS PAS UN NOMBRE");
      apiCall("residential");
